...
def test_rota_visual_nao_inventa_cotovelo_em_linha_diagonal() -> None:
    ...
    assert rota is not None
    cotovelo_horizontal_vertical = (
        len(rota) == 3
        and abs(int(rota[0, 1]) - int(rota[1, 1])) <= 2
        and abs(int(rota[1, 0]) - int(rota[2, 0])) <= 2
    )
    cotovelo_vertical_horizontal = (
        len(rota) == 3
        and abs(int(rota[0, 0]) - int(rota[1, 0])) <= 2
        and abs(int(rota[1, 1]) - int(rota[2, 1])) <= 2
    )
    assert not cotovelo_horizontal_vertical
    assert not cotovelo_vertical_horizontal
