...
class LineFollower():
    ...
    def __turn(self):
        ...
        if self.green_signal == "DOUBLE":
            ...
            motors.run(0, 0, 0.5)
        elif self.green_signal == "LEFT":
            motors.run(30, 10, 0.5)
            motors.run(40, -40, 1.8)
            motors.run(0, 0, 1)
            # self.run_till_camera(30, -30, 20, ["TURNING TILL RIGHT (LEFT)"])
        elif self.green_signal == "RIGHT":
            motors.run(10, 30, 0.5)
            motors.run(-40, 40, 1.8)   
            motors.run(0, 0, 1)
            # self.run_till_camera(-30, 30, 20, ["TURNING TILL RIGHT (LEFT)"])
    ...
    def green_hold(self):
        ...
        elif self.green_signal not in [None, "DOUBLE", "APPROACH"] and self.prev_green_signal != self.green_signal:
            self.last_seen_green = time.perf_counter()

        if time.perf_counter() - self.last_seen_green < 1.2 and self.prev_green_signal not in [None, "DOUBLE", "APPROACH"]:
    ...
    def calculate_angle(self, contour=None, validate=False):
        ...
        if contour is not None and self.green_signal in ["LEFT", "RIGHT"]:
            ...
            point = min(contour, key=lambda p: p[0][0]) if self.green_signal == "RIGHT" else max(contour, key=lambda p: p[0][0])
    ...
    def find_red(self):
        if self.hsv_image is not None:
            mask_lower = cv2.inRange(self.hsv_image, (0, 200, 20), (20, 255, 255))
            mask_upper = cv2.inRange(self.hsv_image, (160, 200, 20), (179, 255, 255))
            mask = cv2.bitwise_or(mask_lower, mask_upper)
            
            print(f"red: {cv2.countNonZero(mask)}, threshold: {int(0.005 * camera.LINE_WIDTH * camera.LINE_HEIGHT)}")
            
            if cv2.countNonZero(mask) > int(0.001 * camera.LINE_WIDTH * camera.LINE_HEIGHT):
