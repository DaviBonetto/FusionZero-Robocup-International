...
def main(start_time, robot_state, line_follow) -> None:
    ...
    if time.perf_counter() - robot_state.red_timer > 1.5:
        robot_state.first_red = False
    
    if line_follow.integral < 100 and robot_state.last_downhill > 200 and (robot_state.first_red == False or time.perf_counter() - robot_state.red_timer > 0.5): 
        robot_state.debug_text.append("READ RED")
        line_follow.find_red()
    
    robot_state.debug_text.append(f"Red: {robot_state.count['red']}, TIMER: {time.perf_counter() - robot_state.red_timer}")
    ...
    if time.perf_counter() - start_time < 3:
    ...
    elif line_follow.integral < 100 and robot_state.count["red"] >= 5 and 0.5 < time.perf_counter() - robot_state.red_timer < 1.5:
        oled_display.text("2nd RED", 10, 10, size=20, clear=True)
        robot_state.debug_text.append("DOUBLE RED")
        motors.run(0, 0, 10)
        
    elif robot_state.count["red"] >= 5 and line_follow.integral < 100:
        oled_display.text("RED", 35, 12, size=30, clear=True)
        robot_state.debug_text.append("FIRST RED")
        robot_state.red_timer = time.perf_counter()
        robot_state.first_red = True
        robot_state.count["red"] = 0
    
    elif robot_state.count["red"] == 0 and robot_state.first_red == False and robot_state.count["silver"] >= 3 and robot_state.trigger["evacuation_zone"] == False:
        ...
        motors.run(30, 30, f)
        motors.run(v1, v2, t)
        line_follow.run_till_camera(v1, v2, 15, ["DOUBLE GREEN"])
