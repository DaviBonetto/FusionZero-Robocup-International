...
class Search():
    def __init__(self, evac_state: EvacuationState, evac_camera: EvacuationCamera):
        ...
        # Live settings
        self.LIVE_THRESHOLD = 247
        self.LIVE_ERODE_KERNAL = np.ones((1, 1), np.uint8)
        self.LIVE_DILATE_KERNAL = np.ones((31, 31), np.uint8)
        
        self.LIVE_UNDILATE_MIN_AREA = 50
        self.LIVE_UNDILATE_MIN_COUNT = 2
        self.LIVE_UNDILATE_MIN_COUNT_MIN_AREA = 2
    ...
    def live(self, image: np.ndarray, display_image: np.ndarray, last_x: Optional[int]) -> Optional[int]:
        ...
        for contour in dilated_contours:
            ...
            if (len(sub_contours) > self.LIVE_UNDILATE_MIN_COUNT and total_area > self.LIVE_UNDILATE_MIN_COUNT_MIN_AREA):
...
class Movement():
    ...
    def wall_follow(self) -> None:
        ...
        distance = laser_sensors.read([2])[0]
...
def analyse(image: np.ndarray, display_image: np.ndarray, search_type: str, last_x: Optional[int] = None) -> tuple[Optional[int], str, Optional[int]]:      
    ...
    if search_type in ["dead", "green", "default"] and dead_enable:
        ...
        if dead_x is not None: return dead_x, "dead", None
    
    if search_type in ["green", "default"] and green_enable:
        x, green_contour_area = op_search.triangle(image, display_image, "green")
        if x is not None: return x, "green", green_contour_area
    
    if search_type in ["red", "default"] and red_enable:
        x, red_contour_area  = op_search.triangle(image, display_image, "red")
        if x is not None: return x, "red", red_contour_area
...
def grab(prev_insert: Optional[int]) -> tuple[bool, int]:    
    ...
    if claw.spaces[0 if insert == -1 else 1] != "":
...
def leave():
    ...
    while listener.mode.value != 0:
        ...
        silver_value = colour_sensors.read()[2]
        gyroscope_value = gyroscope.read()
        
        pitch = None
        if gyroscope_value is not None:
            _, pitch, _ = gyroscope_value
        
        # black_count, silver_count = validate_gap(silver_value, black_count, silver_count)
        colour_value = colour_sensors.read()[2]
        black_count, silver_count = validate_gap(colour_value, black_count, silver_count)
        
        print(f"Counts: {black_count} {silver_count}, pitch {pitch}")

        if pitch is not None:
            uphill_state = pitch > 7
        
        if black_count >= 5 or uphill_state:
...
def main() -> None:
    ...
    while evac_state.victim_count != 1 and listener.mode.value != 0:
        oled_display.clear()
