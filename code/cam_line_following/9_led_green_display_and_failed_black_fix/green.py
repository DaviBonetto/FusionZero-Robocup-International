...
def validate_green_contour(contour, black_image):
    ...
    # Check if there is black above the midpoint
    if 0 <= midpoint[0] - black_height < black_image.shape[1] and 0 <= midpoint[1] - black_height < black_image.shape[0]:
        black_above_midpoint = any(black_image[midpoint[1] - black_height, x] == 255 for x in range(max(0, midpoint[0] - black_height), min(black_image.shape[1], midpoint[0] + black_height+1)))
    else:
