...
try:
    while True:
        ...
        if len(green_contours) == 1:
            valid_rects = validate_green_contour(green_contours[0], black_image)
        elif len(green_contours) > 1:
            valid_rects = validate_multiple_contours(green_contours, black_image)

        green = green_sign(valid_rects, black_image, line_image)
