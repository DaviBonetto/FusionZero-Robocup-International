...
def find(search_function: callable) -> None:
    def search_while(v1: int, v2: int, time_constraint: float, search_function: callable, conditional_function: callable = None) -> int:
        ...
        while time.time() - start_time < time_constraint:
            print(f"(SEARCH WHILE) ({v1}, {v2}) ({search_function}) ({victim_count=})    |    {time.time()-start_time:.2f}")
    ...
    while found_status != 1:
        ...
        if victim_count < 2: found_status = search_while(v1=base_evacuation_speed       , v2=-base_evacuation_speed       , search_function=search_function, time_constraint=time_delay)
        else:                found_status = search_while(v1=base_evacuation_speed * 0.62, v2=-base_evacuation_speed * 0.62, search_function=search_function, time_constraint=time_delay)
...
def route(search_function: callable, kP: float) -> bool:
    ...
    motors.run(0, 0, 0.5)
    motors.run_until(-base_evacuation_speed * 0.62, -base_evacuation_speed * 0.62, laser_sensors.read, 1, ">=", approach_distance, "ROUTE BACK")
...
def align(search_function: callable, step_time: float) -> bool:
    ...
    motors.run(0, 0, 0.3)
    motors.run_until(-base_evacuation_speed * 0.62, -base_evacuation_speed * 0.62, laser_sensors.read, 1, ">=", approach_distance, "ALIGN BACK")
    
    motors.run(0, 0, 0.3)
    motors.run_until( base_evacuation_speed * 0.62,  base_evacuation_speed * 0.62, laser_sensors.read, 1, "<=", approach_distance, "ALIGN FRONT")
