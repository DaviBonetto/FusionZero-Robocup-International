...
def find():
    def align(tolerance: int, text: str, time_step: float = None) -> None:
        ...
        while True:
            ...
            if time_step is None: motors.run(base_evacuation_speed * direction * 0.4, -base_evacuation_speed * direction * 0.4)
            else:
                motors.run(base_evacuation_speed * direction * 0.4, -base_evacuation_speed * direction * 0.4, time_step)
    ...
    motors.run(0, 0, 0.3)
    motors.run_until( base_evacuation_speed,  base_evacuation_speed, laser_sensors.read, 1, "<=", 35)
    motors.run(0, 0, 0.3)
    motors.run_until(-base_evacuation_speed, -base_evacuation_speed, laser_sensors.read, 1, ">=", 35)
