...
def validate_green_contour(contour, black_image):
    ...
    if 0 <= midpoint[0] - 20 < black_image.shape[1] and 0 <= midpoint[1] - 20 < black_image.shape[0]:
        black_above_midpoint = black_image[midpoint[1] - 20, midpoint[0]] == 255
    else:
    ...
    if black_above_midpoint:
        ...
        # print(f"valid_rects {valid_rects}, len {len(valid_rects)} ")
        return valid_rects  # Return the 4 corners of the valid minAreaRect

    return None  # If no black above midpoint, return None

def validate_multiple_contours(contours, black_image):
    if len(contours) > 2:
    ...
    if len(valid_rects) == 2:
        ...
        y_difference = abs((y1 + h1) - (y2 + h2))

        if y_difference < 30:
            # print(f"  rects {valid_rects}, len {len(box)} ")
            return valid_rects
...
def green_sign(valid_rects, black_image, display_image):
    if valid_rects is not None:
        # print(f"len {len(valid_rects)}")
        if len(valid_rects) == 1:   # Note this is 4 points, and not a single rectangle unliek the bottom elif (2 rects)
