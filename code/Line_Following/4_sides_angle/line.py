...
def black_mask(image, display_image):
    black_mask = cv2.inRange(image, (0, 0, 0), (60, 60, 60))

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))  # 3x3 kernel size
...
def calculate_angle(contour, display_image):
    ...
    if contour is not None:
        ...
        top_edge_points = [(p[0][0], p[0][1]) for p in contour if p[0][1] <= 20]
        ...
        if ref_point:
            ...
            if display_image is not None:
                cv2.line(display_image, ref_point, bottom_center, (0, 255, 0), 3)
