...
min_green_area = int(LINE_HEIGHT * LINE_WIDTH/768)
black_LINE_HEIGHT = int(LINE_HEIGHT * 0.12)
...
def validate_green_contour(contour, black_image, display_image, check_distance=10, region_size=3):
    ...
    if edge_length == 0:
        return None, display_image  # Avoid division by zero
...
def validate_multiple_contours(contours, black_image, display_image):
    ...
    for contour in range(len(contours)):
        valid_rect, display_image = validate_green_contour(contours[contour], black_image, display_image)
...
def green_sign(valid_rects, black_image, display_image):
    ...
    if len(valid_rects) == 1:
        ...
        if 0 <= left_check_y < LINE_HEIGHT and 0 <= left_check_x < LINE_WIDTH:
            if black_image[left_check_y, left_check_x] == 255:
                cv2.putText(display_image, ">", (LINE_WIDTH - 55, LINE_HEIGHT // 2 + 25), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 0, 255), 2)
                green_turn = "Right"
                return "Right"
        ...
        if 0 <= right_check_y < LINE_HEIGHT and 0 <= right_check_x < LINE_WIDTH:
            if black_image[right_check_y, right_check_x] == 255:
                cv2.putText(display_image, "<", (0, LINE_HEIGHT // 2 + 25), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 0, 255), 2)
                green_turn = "Left"
                return "Left"
            else:
        ...
        if black_image[mid_y, mid_x] == 255:
            cv2.putText(display_image, "U", (LINE_WIDTH // 2 - 23, LINE_HEIGHT // 2 + 35), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 255), 2)
            green_turn = "U-Turn"
            return "U-Turn"
        else:
