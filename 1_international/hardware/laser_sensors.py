from core.shared_imports import GPIO, board, time, adafruit_vl53l1x, adafruit_vl53l0x
...
class LaserSensors():
    ...
    def __setup(self):
        ...
        for pin_number, sensor in enumerate(self.__tof_sensors):
            if pin_number == 1:
                # VL53L0X uses start_continuous()
                sensor.start_continuous()
            else:
                # VL53L1X uses start_ranging()
    ...
    def __change_address(self, pin_number: int, x_shut_pin: int) -> None:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                if pin_number == 1:
                    # Create VL53L0X sensor for index 1
                    sensor = adafruit_vl53l0x.VL53L0X(board.I2C())
                    # VL53L0X timing budget is in microseconds (20ms = 20000us)
                    sensor.measurement_timing_budget = self.TIMING_BUDGET * 1000
                else:
                    # Create VL53L1X sensor for indices 0 and 2
    ...
    def read(self, pins=None) -> list[int]:
        ...
        values = []
        if pins is None:
            sensors = self.__tof_sensors
            sensor_indices = list(range(len(self.__tof_sensors)))
        else:
            sensors = [self.__tof_sensors[i] for i in pins]
            sensor_indices = pins

        for idx, sensor in zip(sensor_indices, sensors):
            try:
                # VL53L0X uses 'range' property, VL53L1X uses 'distance'
                if idx == 1:
